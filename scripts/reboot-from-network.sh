...
if [[ $REPLY =~ ^[Yy]$ ]]
then
  ssh -t -i ./key.secret -o "StrictHostKeyChecking=no" -o "UserKnownHostsFile=/dev/null" -o "LogLevel=error" "$user@$host" 'sudo efibootmgr -A -b $(efibootmgr | grep "^Boot....\* debian$" | cut -c5-8) && sudo systemctl reboot'
fi
echo # start new line
