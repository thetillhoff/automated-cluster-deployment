...
echo -e "#!ipxe\n" > /container/web/boot.ipxe
echo -e "set boot-url http://$(cut -d' ' -f1 <<<$(hostname -I))/\n" >> /container/web/boot.ipxe
echo -e "initrd \${boot-url}mac/\${mac:hexhyp}_lastinstalled && chain \${boot-url}boot_harddrive.php?mac=\${mac:hexhyp} ||" >> /container/web/boot.ipxe # if already installed boot from harddrive
echo -e "chain \${boot-url}mac/\${mac:hexhyp}.php?mac=\${mac:hexhyp} ||" >> /container/web/boot.ipxe # if mac is known and config file exists, load it
echo -e "chain \${boot-url}default.php?mac=\${mac:hexhyp} ||" >> /container/web/boot.ipxe # else load menu
