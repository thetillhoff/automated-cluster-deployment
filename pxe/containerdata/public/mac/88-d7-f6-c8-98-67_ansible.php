<?php
    ...
    #$mac = str_replace(":", "-", htmlspecialchars($_GET["mac"])); # not required anymore as mac is now in correct format
    $mac = htmlspecialchars($_GET["mac"]);

    $mac_lastinstalled_file = '/container/public/mac/'.$mac.'_lastinstalled';
    $handle = fopen($mac_lastinstalled_file, 'w') or die('Cannot open file:  '.$mac_lastinstalled_file);
    date_default_timezone_set('Europe/Berlin');
    $data = date('Y-m-d H:i:s'); // e.g. 2019-08-16 23:48:00
    fwrite($handle, $data);
?>
...
d-i netcfg/hostname string BEAST
...
d-i passwd/user-fullname string shuroc
d-i passwd/username string shuroc
# Normal user's password, either in clear text
#d-i passwd/user-password password insecure
#d-i passwd/user-password-again password insecure
# or encrypted using a crypt(3) hash.
#d-i passwd/user-password-crypted password [crypt(3) hash]
d-i passwd/user-password-crypted password yftK48L59TcL6
...
d-i pkgsel/include string openssh-server wget git net-tools nano
...
in-target sh -c 'mkdir -p -m 700 /shuroc/.ssh'; \
#   -p creates parent directories as needed
#   -m as chmod
in-target sh -c 'wget --no-proxy -O /home/shuroc/.ssh/authorized_keys https://github.com/shuroc.keys'; \
in-target sh -c 'chmod 600 /home/shuroc/.ssh/authorized_keys'; \
...
## disable password auth via ssh ( -> remote auth works only via certificate and local auth via password still work)
...
# ///TODO
# - add host to managed ansible hosts